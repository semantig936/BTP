...
def _build_features(
    ...
    #sentiment_df = _build_excel_sentiment(prices.index, csv_path)
    sentiment_df = pd.DataFrame(0.0, index=prices.index, columns=["sentiment"])

    # Combine into the final requested frame
    combined = pd.concat(
        [prices, log_returns, volatility, z_score, sentiment_df],
        axis=1,
        keys=["prices", "returns", "vol", "z_score", "sentiment"],
    )
    
    # Requested combined frame
    combined = pd.concat(
        [prices, log_returns, volatility, z_score, sentiment_df],
        axis=1,
        keys=["prices", "returns", "vol", "z_score", "sentiment"],
...
def _save_visualizations(prices: pd.DataFrame, sentiment: pd.DataFrame):
    ...
    plt.plot(sentiment.index, sentiment["sentiment"], label="Market Sentiment")
    plt.title("Macro + Return Sentiment (Market)")
    plt.legend()
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(DATA_STAT_DIR / "market_sentiment.png", dpi=170)
    plt.close()
...
def run_data_pipeline() -> dict:
    ...
    _save_visualizations(artifacts.prices, artifacts.sentiment)
